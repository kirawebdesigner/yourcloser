...
def parse_product_text(text: str) -> dict:
    ...
    if sizes_match:
        ...
        data["sizes"] = [s.strip() for s in sizes_str.split(",") if s.strip()]
    else:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard.append([
        InlineKeyboardButton("🔥 Trending Now", callback_data="discover_trending"),
    ...
    text = (
        ...
        "Tap a discovery collection or category below to start browsing:"
    )
...
async def render_product(update_or_query, context: ContextTypes.DEFAULT_TYPE, edit_current: bool) -> None:
    ...
    caption += "⭐ *Verified Store* ✅ | ⚡ *Fast Delivery*\n"
    if daily_orders > 0:
...
async def admin_add_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    tip_text = (
        ...
        "💡 *WhatsApp-Style Onboarding:* You can also type all details in a single message now, and I will parse it automatically!\n"
        "Example:\n"
